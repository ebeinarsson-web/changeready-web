  ...
  return (
    <section className="relative overflow-hidden rounded-[32px] border border-border/70 bg-[radial-gradient(circle_at_top,rgba(177,122,82,0.14),transparent_55%),linear-gradient(135deg,rgba(20,22,28,0.95),rgba(12,13,16,0.98))] px-6 py-12 shadow-[0_30px_120px_rgba(0,0,0,0.45)] sm:px-10 sm:py-16">
      ...
      <div className="relative space-y-6">
        <p className="text-xs font-semibold uppercase tracking-[0.32em] text-accent">{siteContent.home.eyebrow}</p>
        ...
        <ul className="grid gap-3 text-sm text-muted sm:grid-cols-3">
          {siteContent.home.bullets.map((bullet) => (
            <li
              key={bullet}
              className="rounded-2xl border border-border/60 bg-surface-muted/40 px-4 py-3 leading-relaxed"
            >
              {bullet}
            </li>
          ))}
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <p className="text-sm text-muted sm:max-w-sm">{siteContent.home.secondaryNote}</p>
        </div>
      </div>
    </section>
  );
